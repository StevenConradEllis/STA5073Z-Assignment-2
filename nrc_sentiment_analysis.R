...
#nrc sentiment analysis plots

sona_tdf_nrc %>%
  count(sentiment) %>%
  arrange(desc(n)) %>%
  filter(rank(desc(n)) <= 20) %>%
  ggplot(aes(reorder(sentiment,n),n)) + geom_col() + coord_flip() + xlab('')
...
#
sentiments_per_month <- mandela_nrc  %>%
...
sentiments_per_month %>% filter(sentiment == 'anger') %>%
  ggplot(aes(x = year, y = freq, colour = sentiment, group=sentiment)) +
  geom_line() + 
  geom_smooth(aes(colour = sentiment))
